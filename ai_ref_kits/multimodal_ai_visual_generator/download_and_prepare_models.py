...
def prepare_llm_model():
    if not Path(model_dir+"/llama-3.1-8b-instruct").exists():
        print("llama Model not downloaded.")
        cmd = "optimum-cli export openvino --model meta-llama/Llama-3.1-8B-Instruct --task text-generation-with-past --weight-format int4 --group-size -1 --ratio 1.0 --sym "  + model_dir + "/llama-3.1-8b-instruct/INT4_compressed_weights"
